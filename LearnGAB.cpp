...
bool GAB::NPDClassify(Mat test,float &score, int sIndex){
  ...
  for(int i = 0 ;i<stages;i++){
    ...
    while(node > -1){
      unsigned char p1 = *(I+points1[sIndex][node]*test.step[0]+points1[sIndex][node]);
      unsigned char p2 = *(I+points2[sIndex][node]*test.step[0]+points2[sIndex][node]);
...
vector<int> GAB::DetectFace(Mat img, vector<Rect>& rects, vector<float>& scores){
	...
	int minFace = 24; // changed from 20 to 24
	int maxFace = 3000; //changed from 3000 to 4000
...
//	omp_set_num_threads(opt.numThreads);
	...
	for (int k = 0; k < 29; k++) // process each scale n = 29 number of scales
	{
...
vector<int> GAB::DetectFace2(cv::Mat img, vector<cv::Rect>& rects, vector<float>& scores){
	const Options& opt = Options::GetInstance();
